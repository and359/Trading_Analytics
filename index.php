...
<!--get data from mysql-->
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$pxETSY = $pxETSY . '"'. $row['Price'].'",';
		$dateETSY = $dateETSY . '"'. $row['PriceDate'] .'",';
	}
	
	$pxETSY = trim($pxETSY,",");
	$dateETSY = trim($dateETSY,",");
	//end of ETSY
	
	
	//TSLA
	$sql = "SELECT * FROM `backtest` WHERE Ticker = 'TSLA';";
    	$result = mysqli_query($mysqli, $sql);

	//loop through the returned data
	while ($row = mysqli_fetch_array($result)) {

		$pxTSLA = $pxTSLA . '"'. $row['Price'].'",';
		$dateTSLA = $dateTSLA . '"'. $row['PriceDate'] .'",';
	}
	
	$pxTSLA = trim($pxTSLA,",");
	$dateTSLA = trim($dateTSLA,",");
	//end of TSLA
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$pxIVW = $pxIVW . '"'. $row['Price'].'",';
		$dateIVW = $dateIVW . '"'. $row['PriceDate'] .'",';
	}
	...
	<ul class="sidenav sidenav-fixed bg" id="slide-out">
		<li>
			<div class="user-view">
				...
				<a href="#" class="white-text name">Andy Li (CA, CFA, FMVA, FRM)</a>
					...
					<div class="card-content">
						...
						<!--<input type="button" name="tweet_button" id="tweet_button"  value="Tweet" class="btn btn-info" />-->
							...
							var testIVW = marketingIVW.map(function(dateIVWv, indexIVW) {
								...
								var b4ifdateETSY = [<?php echo $dateETSY; ?>];
								var b4ifpxETSY = [<?php echo $pxETSY; ?>];
								var b4ifdateTSLA = [<?php echo $dateTSLA; ?>];
								var b4ifpxTSLA = [<?php echo $pxTSLA; ?>];
								var b4ifdateIVW = [<?php echo $dateIVW; ?>];
								var b4ifpxIVW = [<?php echo $pxIVW; ?>];
